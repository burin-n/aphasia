...
#--splice-indexes "layer0/-2:-1:0:1:2 layer1/-1:2 layer3/-3:3 layer4/-7:2" \
  steps/nnet2/train_multisplice_accel2.sh --stage $train_stage \
    --feat-type raw \
    --splice-indexes "layer0/-2:-1:0:1:2 layer1/-1:2 layer3/-3:3" \
    --num-epochs 16 \
    --num-hidden-layers 4 \
...
if [ $stage -le 8 ]; then
  ...
  # previous utterances of the same speaker.
   steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 12 \
      exp/tri5a/graph data/test ${dir}_online/decode_test || exit 1;
fi
echo ZZZZ
if [ $stage -le 9 ]; then
  ...
  # without carrying forward speaker information.
   steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 12 \
     --per-utt true \
      exp/tri5a/graph data/test ${dir}_online/decode_test_utt || exit 1;
fi
echo TTTTTTTTTT
if [ $stage -le 10 ]; then
  # this version of the decoding treats each utterance separately
  ...
  # of the utterance while computing the iVector.
   steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 12 \
     --per-utt true --online false \
      exp/tri5a/graph data/test ${dir}_online/decode_test_utt_offline || exit 1;
fi
